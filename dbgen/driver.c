...
int partial(int tbl, int s) {
  ...
  if (verbose > 0) {
    fprintf(stderr, "\tStarting to load stage %d of %ld for %s...", s, children,
            tdefs[tbl].comment);
  }
...
int pload(int tbl) {
  ...
  if (verbose > 0) {
    fprintf(stderr, "Starting %ld children to load %s", children,
            tdefs[tbl].comment);
  }
  ...
  */
  if (updates) {
    ...
    while (upd_num < updates) {
      if (verbose > 0)
#ifdef SSBM
        fprintf(stderr, "Generating update pair #%ld for %s [pid: %d]",
  ...
  **/
  ...
  for (i = PART; i <= REGION; i++)
    if (table & (1 << i)) {
      ...
      if (validate) {
        printf("Validation checksum for %s ", tdefs[i].name); // shit

        printf("at %lu GB: ", scale);      // shit
        printf("%0lx\n", tdefs[i].vtotal); // shit
      }
    }
