...
int dbg_print_char(int format, FILE *target, char data, int sep) {
...
int dbg_print_charp(int format, FILE *target, char *data, int len, int sep) {
  ...
  case DT_VSTR:
    ...
    fprintf(target, "%c%c%-*s", (len >> 8) & 0xFF, len & 0xFF, len, data);
    break;
#endif /* MVS */
  default:
    abort();
...
int dbg_print_long(int format, FILE *target, long data, int len, int sep) {
  int dollars, cents;
...
int dbg_print_longp(int format, FILE *target, long *data, int len, int sep) {
